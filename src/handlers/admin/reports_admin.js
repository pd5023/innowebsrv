...
async function listReports({ cltId, cntId, from, to } = {}) {
  const r = await pool.query(
    `SELECT sr.subr_id, sr.sr_date, sr.sr_repairs, sr.sr_complete, sr.sr_sign_name,
...
async function getReport(id) {
  const r = await pool.query('SELECT * FROM sub_reports WHERE subr_id = $1', [id]);
  if (!r.rows.length) return null;
  const hours  = await pool.query('SELECT * FROM sr_hours  WHERE subr_id = $1', [id]);
  const eparts = await pool.query('SELECT * FROM sr_parts  WHERE subr_id = $1', [id]);
  const equips = await pool.query('SELECT * FROM sr_equips WHERE subr_id = $1', [id]);
